...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subOdometry = nh.subscribe<nav_msgs::Odometry>
                                ("/lio_sam/mapping/odometry", 5, odometryHandler);

  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>
                                  ("/lio_sam/deskew/cloud_registered", 5, laserCloudHandler);
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.frame_id = "drivebase/base_footprint";
          ...
          freePaths2.header.frame_id = "drivebase/base_footprint";
